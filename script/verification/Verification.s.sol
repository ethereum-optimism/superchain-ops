...
contract SuperchainRegistry is CommonBase {
    ...
    constructor(
        string memory _l1ChainName,
        string memory _l2ChainName,
        string memory _opContractsRelease
    ) {
        l1ChainName = _l1ChainName;
        l2ChainName = _l2ChainName;
        opContractsReleaseQ = string.concat(
            '"op-contracts/',
            _opContractsRelease,
            '"'
        );
        try vm.envString("SCR_OVERRIDE_CHAIN_CONFIG_PATH") returns (
            string memory path
        ) {
            console.log(
                "SuperchainRegistry: overriding chain config path to %s",
                path
            );
            chainConfigPath = path;
        } catch {
            // default path
            chainConfigPath = string.concat(
    ...
    function _readSuperchainConfig() internal {
        ...
        proxies.SuperchainConfig = stdToml.readAddress(
            toml,
            "$.superchain_config_addr"
        );
        proxies.ProtocolVersions = stdToml.readAddress(
            toml,
            "$.protocol_versions_addr"
        );

        try
            vm.readFile(string.concat(vm.projectRoot(), chainConfigPath))
        returns (string memory data) {
            toml = data;
        } catch {
            revert(string.concat("Failed to read ", chainConfigPath));
        }
        proxies.OptimismPortal = stdToml.readAddress(
            toml,
            "$.addresses.OptimismPortalProxy"
        );
        proxies.L1CrossDomainMessenger = stdToml.readAddress(
            toml,
            "$.addresses.L1CrossDomainMessengerProxy"
        );
        proxies.L1StandardBridge = stdToml.readAddress(
            toml,
            "$.addresses.L1StandardBridgeProxy"
        );
        proxies.SystemConfig = stdToml.readAddress(
            toml,
            "$.addresses.SystemConfigProxy"
        );
        ...
        proxies.AnchorStateRegistry = stdToml.readAddressOr(
            toml,
            "$.addresses.AnchorStateRegistryProxy",
            address(0)
        );
        proxies.DisputeGameFactory = stdToml.readAddressOr(
            toml,
        ...
        chainConfig.systemConfigOwner = stdToml.readAddress(
            toml,
            "$.roles.SystemConfigOwner"
        );
        chainConfig.unsafeBlockSigner = stdToml.readAddressOr(
            toml,
            "$.roles.UnsafeBlockSigner",
            address(0)
        ); // Not present on all chains, note .readAddressOr
        chainConfig.batchSubmitter = stdToml.readAddress(
            toml,
            "$.roles.BatchSubmitter"
        );
        chainConfig.batchInbox = stdToml.readAddress(
            toml,
            "$.batch_inbox_addr"
        );
    }
    ...
    function _readStandardVersions() internal {
        string memory toml;
        string memory path = string.concat(
            "/lib/superchain-registry/validation/standard/standard-versions-",
            l1ChainName,
            ".toml"
        );
        try vm.readFile(string.concat(vm.projectRoot(), path)) returns (
        ...
        standardVersions.SystemConfig = _parseStandardVersionImpl(
            toml,
            "system_config"
        );
        standardVersions.FaultDisputeGame = _parseStandardVersion(
            toml,
            "fault_dispute_game"
        );
        standardVersions.PermissionedDisputeGame = _parseStandardVersion(
            toml,
            "permissioned_dispute_game"
        );
        standardVersions.MIPS = _parseStandardVersionAddr(toml, "mips");
        standardVersions.OptimismPortal = _parseStandardVersionImpl(
            toml,
            "optimism_portal"
        );
        standardVersions.AnchorStateRegistry = _parseStandardVersion(
            toml,
            "anchor_state_registry"
        );
        standardVersions.DelayedWETH = _parseStandardVersionImpl(
            toml,
            "delayed_weth"
        );
        standardVersions.DisputeGameFactory = _parseStandardVersionImpl(
            toml,
            "dispute_game_factory"
        );
        standardVersions.PreimageOracle = _parseStandardVersionAddr(
            toml,
            "preimage_oracle"
        );
        standardVersions.L1CrossDomainMessenger = _parseStandardVersionImpl(
            toml,
            "l1_cross_domain_messenger"
        );
        standardVersions.L1ERC721Bridge = _parseStandardVersionImpl(
            toml,
            "l1_erc721_bridge"
        );
        standardVersions.L1StandardBridge = _parseStandardVersionImpl(
            toml,
            "l1_standard_bridge"
        );
        standardVersions
            .OptimismMintableERC20Factory = _parseStandardVersionImpl(
            toml,
            "optimism_mintable_erc20_factory"
        );
    }

    function _parseStandardVersionImpl(
        string memory data,
        string memory key
    ) internal pure returns (StandardVersionImpl memory sv_) {
        sv_.version = stdToml.readString(
            data,
            string.concat("$.RELEASE.", key, ".version")
        );
        sv_.implementation = stdToml.readAddress(
            data,
            string.concat("$.RELEASE.", key, ".implementation_address")
        );
    }

    function _parseStandardVersionAddr(
        string memory data,
        string memory key
    ) internal pure returns (StandardVersionAddr memory sv_) {
        sv_.version = stdToml.readString(
            data,
            string.concat("$.RELEASE.", key, ".version")
        );
        sv_.Address = stdToml.readAddress(
            data,
            string.concat("$.RELEASE.", key, ".address")
        );
    }

    function _parseStandardVersion(
        string memory data,
        string memory key
    ) internal pure returns (StandardVersion memory sv_) {
        sv_.version = stdToml.readString(
            data,
            string.concat("$.RELEASE.", key, ".version")
        );
    ...
    function _applyOverrides() internal {
        try vm.envAddress("SCR_OVERRIDE_MIPS_ADDRESS") returns (address mips) {
            console.log(
                "SuperchainRegistry: overriding MIPS address to %s",
                mips
            );
        ...
        try vm.envString("SCR_OVERRIDE_MIPS_VERSION") returns (
            string memory ver
        ) {
            console.log(
                "SuperchainRegistry: overriding MIPS version to %s",
                ver
            );
