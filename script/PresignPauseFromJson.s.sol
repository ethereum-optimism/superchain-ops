...
contract PresignPauseFromJson is MultisigBuilder, JsonTxBuilderBase {
        ...
        returns (SimulationStateOverride memory override_)
    {
        ...
        if (
            vm.envOr("SIMULATE_WITHOUT_LEDGER", false) ||
            vm.envOr("SIMULATE_WITHOUT_LEDGER", uint256(0)) == 1
        ) {
            console.log("Adding override for test sender");
            uint256 nonce = _getNonce(IGnosisSafe(_ownerSafe()));
            override_ = overrideSafeThresholdOwnerAndNonce(
                _ownerSafe(),
                vm.envAddress("TEST_SENDER"),
                nonce
            );
        }
    }
        ...
        returns (IMulticall3.Call3[] memory)
    {
        string memory jsonContent = vm.readFile(
            vm.envOr("INPUT_JSON_PATH", string("input.json"))
        );
    ...
    function _postCheck(
        ...
        for (uint256 i; i < accesses.length; i++) {
            ...
            for (uint256 j; j < accountAccess.storageAccesses.length; j++) {
                Vm.StorageAccess memory storageAccess = accountAccess
                    .storageAccesses[j];
                ...
                if (storageAccess.isWrite) {
                    ...
                    if (storageAccess.account == _ownerSafe()) {
                        require(
                            storageAccess.slot == bytes32(uint256(5)),
                            "The only allowed write is to the nonce"
                        );
                        require(
                            uint256(storageAccess.previousValue) ==
                                uint256(storageAccess.newValue) - 1,
                            "The nonce should have increased by 1"
                        );
                    } else if (
                        storageAccess.account ==
                        vm.envAddress("SUPERCHAIN_CONFIG_ADDR")
                    ) {
