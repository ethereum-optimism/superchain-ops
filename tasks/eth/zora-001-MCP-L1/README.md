...
This is the playbook for executing Upgrade #6, Zora Mainnet Multi-Chain Prep (MCP) L1.
...
- [X] Executed on Mode Sepolia: https://github.com/ethereum-optimism/superchain-ops/tree/57b3450cd0d870dc0c8b4d96980747eb6274e3ba/tasks/sep/mode-001-MCP-L1
...
When simulating, ensure the logs say `Using script Using script /your/path/to/superchain-ops/tasks/eth/zora-001-MCP-L1/SignFromJson.s.sol`.
...
When executing, ensure the logs say `Using script Using script /your/path/to/superchain-ops/tasks/eth/zora-001-MCP-L1/SignFromJson.s.sol`.
Thus ensures all safety checks are run. If the default `SignFromJson.s.sol` script is shown
(without the full path), something is wrong and the safety checks will not run.