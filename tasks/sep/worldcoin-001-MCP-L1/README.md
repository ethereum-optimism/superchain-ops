# Worldcoin Sepolia MCP L1 Upgrade

Status: DRAFT, NOT READY TO SIGN

## Objective

This is the playbook for executing Upgrade #6, Multi-Chain Prep (MCP) L1, on Worldcoin Sepolia (chain ID 4801).
...
When simulating, ensure the logs say `Using script Using script /your/path/to/superchain-ops/tasks/sep/worldcoin-001-MCP-L1/SignFromJson.s.sol`.
...
When executing, ensure the logs say `Using script Using script /your/path/to/superchain-ops/tasks/sep/worldcoin-001-MCP-L1/SignFromJson.s.sol`.
Thus ensures all safety checks are run. If the default `SignFromJson.s.sol` script is shown
(without the full path), something is wrong and the safety checks will not run.
