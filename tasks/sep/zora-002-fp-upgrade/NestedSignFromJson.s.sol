...
import {ISuperchainConfig} from "@eth-optimism-bedrock/interfaces/L1/ISuperchainConfig.sol";
...
contract NestedSignFromJson is OriginalNestedSignFromJson, CouncilFoundationNestedSign, SuperchainRegistry {
    ...
    /// @notice OP Sepolia address for the AnchorStateRegistry implementation for comparison.
    ...
    function getAllowedStorageAccess() internal view override returns (address[] memory allowed_) {
        allowed_ = new address[](10);
        allowed_[0] = vm.envAddress("OWNER_SAFE");
        allowed_[1] = vm.envAddress("FOUNDATION_SAFE");
        allowed_[2] = vm.envAddress("COUNCIL_SAFE");
        allowed_[3] = proxies.OptimismPortal;
        allowed_[4] = proxies.SystemConfig;
        allowed_[5] = addressManager;
        allowed_[6] = proxies.L1CrossDomainMessenger;
        allowed_[7] = proxies.L1StandardBridge;
        allowed_[8] = proxies.L1ERC721Bridge;
        allowed_[9] = proxies.OptimismMintableERC20Factory;
    }
    ...
    function checkAnchorStateRegistry() internal {
        ...
        diffs[0] =
            BytecodeComparison.Diff({start: 399, content: abi.encodePacked(address(expectedDisputeGameFactoryProxy))});
        diffs[1] =
            BytecodeComparison.Diff({start: 840, content: abi.encodePacked(address(expectedDisputeGameFactoryProxy))});
        diffs[2] =
            BytecodeComparison.Diff({start: 2308, content: abi.encodePacked(address(expectedDisputeGameFactoryProxy))});
        require(
    ...
    function checkPermissionedDisputeGame() internal view {
        ...
        diffs[0] = BytecodeComparison.Diff({start: 1353, content: abi.encodePacked(expectedDelayedWETHProxy)});
        diffs[1] = BytecodeComparison.Diff({start: 1423, content: abi.encodePacked(chainConfig.challenger)});
        diffs[2] = BytecodeComparison.Diff({start: 1640, content: abi.encodePacked(expectedAnchorStateRegistryProxy)});
        diffs[3] = BytecodeComparison.Diff({start: 2266, content: abi.encodePacked(chainConfig.proposer)});
        diffs[4] = BytecodeComparison.Diff({start: 2742, content: abi.encodePacked(uint32(chainConfig.chainId))});
        diffs[5] = BytecodeComparison.Diff({start: 6162, content: abi.encodePacked(expectedAnchorStateRegistryProxy)});
        diffs[6] = BytecodeComparison.Diff({start: 6612, content: abi.encodePacked(expectedDelayedWETHProxy)});
        diffs[7] = BytecodeComparison.Diff({start: 6882, content: abi.encodePacked(chainConfig.proposer)});
        diffs[8] = BytecodeComparison.Diff({start: 6945, content: abi.encodePacked(chainConfig.challenger)});
        diffs[9] = BytecodeComparison.Diff({start: 7088, content: abi.encodePacked(chainConfig.proposer)});
        diffs[10] = BytecodeComparison.Diff({start: 8322, content: abi.encodePacked(chainConfig.proposer)});
        diffs[11] = BytecodeComparison.Diff({start: 8385, content: abi.encodePacked(chainConfig.challenger)});
        diffs[12] = BytecodeComparison.Diff({start: 9583, content: abi.encodePacked(uint32(chainConfig.chainId))});
        diffs[13] = BytecodeComparison.Diff({start: 10810, content: abi.encodePacked(expectedDelayedWETHProxy)});
        diffs[14] = BytecodeComparison.Diff({start: 13611, content: abi.encodePacked(expectedDelayedWETHProxy)});
        diffs[15] = BytecodeComparison.Diff({start: 13958, content: abi.encodePacked(expectedAnchorStateRegistryProxy)});
        diffs[16] = BytecodeComparison.Diff({start: 14984, content: abi.encodePacked(expectedDelayedWETHProxy)});
        require(
            BytecodeComparison.compare(
...
library BytecodeComparison {
        ...
        returns (bool)
    {
        ...
        Diff[] memory diffs = new Diff[](_bytecodeA.length);
        uint256 diffCount = 0;
        uint256 i = 0;
        while (i < _bytecodeA.length) {
            if (_bytecodeA[i] != _bytecodeB[i]) {
                uint256 start = i;
                uint256 j = i;
                while (j < _bytecodeA.length && _bytecodeA[j] != _bytecodeB[j]) {
                    j++;
                }

                bytes memory diffContent = new bytes(j - i);
                for (uint256 k = i; k < j; k++) {
                    diffContent[k - i] = _bytecodeB[k];
                }

                diffs[diffCount] = Diff({start: start, content: diffContent});
                diffCount++;
                i = j;
            } else {
                i++;
        ...
        for (uint256 d = 0; d < diffCount; d++) {
            bool allowedFound = false;
            for (uint256 a = 0; a < _allowed.length; a++) {
                if (
                    diffs[d].start == _allowed[a].start && keccak256(diffs[d].content) == keccak256(_allowed[a].content)
                ) {
                    allowedFound = true;
                    break;
                }
            ...
            if (!allowedFound) {
                revert(
                    string.concat(
                        "BytecodeComparison: unexpected diff found, start:",
                        LibString.toString(diffs[d].start),
                        ", content:",
                        LibString.toHexString(diffs[d].content)
                    )
                );
            }
        }
