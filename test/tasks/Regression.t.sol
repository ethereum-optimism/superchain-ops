...
contract RegressionTest is Test {
    ...
    /// @notice expected call data and data to sign generated by manually running the WelcomeToSuperchainOps at block 22891231 on mainnet using script:
    /// forge script test/template/WelcomeToSuperchainOps.sol --sig "simulate(string)" test/tasks/example/eth/011-welcome-to-superchain-ops/config.toml --rpc-url mainnet --fork-block-number 22891231 -vv
    function testRegressionCallDataMatches_WelcomeToSuperchainOps() public {
        ...
        address[] memory allSafes = MultisigTaskTestHelper.getAllSafes(rootSafe);
        (Action[] memory actions, uint256[] memory allOriginalNonces) =
            _setupAndSimulate(taskConfigFilePath, 22891231, "mainnet", multisigTask, allSafes);
