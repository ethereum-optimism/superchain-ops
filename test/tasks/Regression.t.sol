...
contract RegressionTest is Test {
    ...
    /// using script:
    /// forge script TestOPCMUpgradeVxyz --sig "simulateRun(string)" src/improvements/tasks/example/sep/004-test-opcm-upgrade-v-xyz/config.toml --rpc-url sepolia --fork-block-number 7826712 -vv
    function testRegressionCallDataMatches_OPCMUpgradeVxyz() public {
        string memory taskConfigFilePath = "src/improvements/tasks/example/sep/004-test-opcm-upgrade-v-xyz/config.toml";
        // call data generated by manually running the TestOPCMUpgradeVxyz template at block 7834423 on sepolia
        string memory expectedCallData =
        ...
        MultisigTask.Action[] memory actions =
            _setupAndSimulateRun(taskConfigFilePath, 7834423, "sepolia", multisigTask);

        _assertCallData(multisigTask, actions, expectedCallData);

        // data to sign generated by manually running the TestOPCMUpgradeVxyz template at block 7834423 on sepolia
        // for each child multisig
        string[] memory expectedDataToSign = new string[](2);
        expectedDataToSign[0] =
