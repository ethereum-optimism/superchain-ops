...
contract RegressionTest is Test {
    ...
    /// @notice Expected call data and data to sign generated by simulating OPCMUpgradeV700 against
    /// u19-beta-0 (chainId 420120035) at sepolia block 10808500 in the PERMISSIONED configuration
    /// (startingRespectedGameType = 1). Companion to the permissionless case above: this exercises
    /// the permissioned path of OPCMUpgradeV700 — CANNON_KONA (game type 8) is skipped
    /// (`enabled=false`, slot stays zero), the kona prestate MUST be the zero address, and the
    /// standard validator reports CKDG-NOSHAPE/CKDG-10 (matched by the example's
    /// expectedValidationErrors). Simulate from
    /// test/tasks/example/sep/038-opcm-upgrade-v700-permissioned with:
    ...
    function testRegressionCallDataMatches_OPCMUpgradeV700_U19_Permissioned() public {
        ...
        // (CANNON, CANNON_KONA) and 3-6 are disabled stubs. startingRespectedGameType = 1.
        string memory expectedCallData =
