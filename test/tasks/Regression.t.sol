...
import {UpdateSystemConfigGasParams} from "src/improvements/template/UpdateSystemConfigGasParams.sol";
...
contract RegressionTest is Test {
    ...
    /// @notice Expected call data and data to sign generated by manually running the UpdateSystemConfigGasParams template at block 8044263 on sepolia.
    /// Simulate from task directory (test/tasks/example/eth/006-update-system-config-gas-params) with:
    /// SIMULATE_WITHOUT_LEDGER=1 just --dotenv-path $(pwd)/.env --justfile ../../../../../src/improvements/single.just simulate
    function testRegressionCallDataMatches_UpdateSystemConfigGasParams() public {
        string memory taskConfigFilePath = "test/tasks/example/eth/006-update-system-config-gas-params/config.toml";
        // Call data generated by manually running the UpdateSystemConfigGasParams template at block 22283936 on mainnet
        string memory expectedCallData =
        ...
        MultisigTask multisigTask = new UpdateSystemConfigGasParams();
        address foundationChildMultisig = 0x847B5c174615B1B7fDF770882256e2D3E95b9D92;
        MultisigTask.Action[] memory actions =
            _setupAndSimulateRun(taskConfigFilePath, 22283936, "mainnet", multisigTask, foundationChildMultisig);
