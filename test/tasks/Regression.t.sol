...
import {EnableLivenessModule2Template} from "src/improvements/template/EnableLivenessModule2Template.sol";
import {DisableLivenessModule2Template} from "src/improvements/template/DisableLivenessModule2Template.sol";
...
contract RegressionTest is Test {
    ...
    /// @notice expected call data and data to sign generated by manually running the example task test/tasks/example/sep/024-enable-liveness-module2:
    /// cd test/tasks/example/sep/024-enable-liveness-module2
    /// SIMULATE_WITHOUT_LEDGER=1 just --dotenv-path $(pwd)/.env --justfile ../../../../../src/improvements/justfile simulate council
    function testRegressionCallDataMatches_EnableLivenessModule2Template() public {
        string memory taskConfigFilePath = "test/tasks/example/sep/024-enable-liveness-module2/config.toml";
        string memory expectedCallData =
            "";
        MultisigTask multisigTask = new EnableLivenessModule2Template();
        address rootSafe = address(0x7a50f00e8D05b95F98fE38d8BeE366a7324dCf7E); // GuardianSafe
        address[] memory allSafes = MultisigTaskTestHelper.getAllSafes(rootSafe);
        (Action[] memory actions, uint256[] memory allOriginalNonces) =
            _setupAndSimulate(taskConfigFilePath, 9020234, "sepolia", multisigTask, allSafes);
    ...
    /// @notice expected call data and data to sign generated by manually running the example task test/tasks/example/sep/025-disable-liveness-module2:
    /// cd test/tasks/example/sep/025-disable-liveness-module2
    /// SIMULATE_WITHOUT_LEDGER=1 just --dotenv-path $(pwd)/.env --justfile ../../../../../src/improvements/justfile simulate council
    function testRegressionCallDataMatches_DisableLivenessModule2Template() public {
        string memory taskConfigFilePath = "test/tasks/example/sep/025-disable-liveness-module2/config.toml";
        string memory expectedCallData =
            "";
        MultisigTask multisigTask = new DisableLivenessModule2Template();
        address rootSafe = address(0x7a50f00e8D05b95F98fE38d8BeE366a7324dCf7E); // GuardianSafe
        address[] memory allSafes = MultisigTaskTestHelper.getAllSafes(rootSafe);
        (Action[] memory actions, uint256[] memory allOriginalNonces) =
            _setupAndSimulate(taskConfigFilePath, 9020234, "sepolia", multisigTask, allSafes);
