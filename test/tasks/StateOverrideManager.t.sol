...
contract StateOverrideManagerUnitTest is Test {
    ...
    function testUserTenderlyStateOverridesTakePrecedence() public {
        ...
        Simulation.StateOverride[] memory noStateOverrides =
            assertDefaultStateOverrides(expectedNonce, 2, noStateOverridesTask, SECURITY_COUNCIL_CHILD_MULTISIG, 0);
        assertEq(noStateOverrides[0].overrides[1].key, bytes32(uint256(5)), "User defined override key must be 5");
        assertEq(
            noStateOverrides[0].overrides[1].value,
        ...
        Simulation.StateOverride[] memory allOverrides =
            assertDefaultStateOverrides(expectedUserOverrideNonce, 2, task, SECURITY_COUNCIL_CHILD_MULTISIG, 0);
        ...
        removeFile(noStateOverridesFileName);
        removeFile(fileName);
    }
    ...
    function testAdditionalUserStateOverridesApplied() public {
        ...
        assertEq(allOverrides[0].overrides[2].key, overrideKey, "User override key must match expected value");
        assertEq(allOverrides[0].overrides[2].value, bytes32(uint256(9999)), "User override must be applied last");
        removeFile(fileName);
    }
    ...
    function testMultipleAddressStateOverridesApplied() public {
        ...
        assertEq(
            allOverrides[0].overrides[2].key, overrideKey, "First address user override key must match expected value"
        );
        assertEq(
            allOverrides[0].overrides[2].value, bytes32(uint256(9999)), "First address user override must be applied"
        );
        assertEq(
