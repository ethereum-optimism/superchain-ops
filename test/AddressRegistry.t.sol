...
import {BASE_CHAIN_ID, OPTIMISM_CHAIN_ID} from "src/fps/utils/Constants.sol";
...
contract MainnetAddressRegistryTest is Test {
    ...
    function testContractState() public view {
        ...
        assertTrue(addresses.supportedL2ChainIds(OPTIMISM_CHAIN_ID), "Optimism chain ID not supported");
    ...
    function testLocalAddressesLoaded() public view {
        assertEq(
            addresses.getAddress("DEPLOYER_EOA", OPTIMISM_CHAIN_ID),
            0x9679E26bf0C470521DE83Ad77BB1bf1e7312f739,
            "DEPLOYER_EOA address mismatch"
        );
        assertEq(
            addresses.getAddress("COMPOUND_GOVERNOR_BRAVO", OPTIMISM_CHAIN_ID),
            0xc0Da02939E1441F497fd74F78cE7Decb17B66529,
            "COMPOUND_GOVERNOR_BRAVO address mismatch"
        );
        assertEq(
            addresses.getAddress("COMPOUND_CONFIGURATOR", OPTIMISM_CHAIN_ID),
            0x316f9708bB98af7dA9c68C1C3b5e79039cD336E3,
            "COMPOUND_CONFIGURATOR address mismatch"
        );

        assertTrue(
            addresses.isAddressContract("COMPOUND_GOVERNOR_BRAVO", OPTIMISM_CHAIN_ID),
            "Address governor bravo should be a contract"
        );
        assertTrue(
            addresses.isAddressContract("COMPOUND_CONFIGURATOR", OPTIMISM_CHAIN_ID),
            "Address configurator should be a contract"
        );
        assertFalse(
            addresses.isAddressContract("DEPLOYER_EOA", OPTIMISM_CHAIN_ID), "EOA address should not be a contract"
        );

        assertTrue(
            addresses.isAddressRegistered("DEPLOYER_EOA", OPTIMISM_CHAIN_ID), "DEPLOYER_EOA should be registered"
        );
        assertTrue(
            addresses.isAddressRegistered("COMPOUND_GOVERNOR_BRAVO", OPTIMISM_CHAIN_ID),
            "COMPOUND_GOVERNOR_BRAVO should be registered"
        );
        assertTrue(
            addresses.isAddressRegistered("COMPOUND_CONFIGURATOR", OPTIMISM_CHAIN_ID),
            "COMPOUND_CONFIGURATOR should be registered"
        );
        assertFalse(
            addresses.isAddressRegistered("NON_EXISTENT_ADDRESS", OPTIMISM_CHAIN_ID),
    ...
    function testSuperchainAddressesLoaded() public view {
        assertEq(
            addresses.getAddress("OptimismPortalProxy", OPTIMISM_CHAIN_ID),
            0xbEb5Fc579115071764c7423A4f12eDde41f106Ed,
            "OP Portal address mismatch"
        );
        assertEq(
            addresses.getAddress("L1StandardBridgeProxy", OPTIMISM_CHAIN_ID),
            0x99C9fc46f92E8a1c0deC1b1747d010903E884bE1,
            "OP Bridge address mismatch"
        );
        assertEq(
            addresses.getAddress("L1CrossDomainMessengerProxy", OPTIMISM_CHAIN_ID),
            0x25ace71c97B33Cc4729CF772ae268934F7ab5fA1,
            "OP Messenger address mismatch"
        );

        assertTrue(
            addresses.isAddressContract("OptimismPortalProxy", OPTIMISM_CHAIN_ID), "OP Portal should be a contract"
        );
        assertTrue(
            addresses.isAddressContract("L1StandardBridgeProxy", OPTIMISM_CHAIN_ID), "OP Bridge should be a contract"
        );
        assertTrue(
            addresses.isAddressContract("L1CrossDomainMessengerProxy", OPTIMISM_CHAIN_ID),
            "OP Messenger should be a contract"
        );

        assertTrue(
            addresses.isAddressRegistered("OptimismPortalProxy", OPTIMISM_CHAIN_ID), "OP Portal should be registered"
        );
        assertTrue(
            addresses.isAddressRegistered("L1StandardBridgeProxy", OPTIMISM_CHAIN_ID), "OP Bridge should be registered"
        );
        assertTrue(
            addresses.isAddressRegistered("L1CrossDomainMessengerProxy", OPTIMISM_CHAIN_ID),
    ...
    function testGetNonExistentAddressFails() public {
        ...
        addresses.getAddress("NON_EXISTENT_ADDRESS", OPTIMISM_CHAIN_ID);
    }
    ...
    function testGetIsAddressContractNonExistentAddressFails() public {
        ...
        addresses.isAddressContract("NON_EXISTENT_ADDRESS", OPTIMISM_CHAIN_ID);
    }
