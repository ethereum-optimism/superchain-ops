...
contract StateDiffChecker_Test is Test {
    ...
    function test_parseDiffSpecs() public {
        ...
        assertEq(diffSpec.chainId, 1337);
        assertEq(diffSpec.storageAccesses.length, 3);
        assertEq(diffSpec.storageAccesses[0].slot, bytes32(0));
        assertEq(diffSpec.storageAccesses[0].newValue, bytes32(0));
        assertEq(diffSpec.storageAccesses[1].slot, bytes32(uint256(1)));
        assertEq(diffSpec.storageAccesses[1].newValue, bytes32(uint256(uint160(address(0xabba)))));
        assertEq(diffSpec.storageAccesses[2].slot, keccak256(abi.encodePacked(uint256(1), uint256(2))));
        assertEq(diffSpec.storageAccesses[2].newValue, hex"acdc");
    }
