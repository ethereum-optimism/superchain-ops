...
contract SuperRootMigrateIntegrationTest is Test, OPCMMigrateV800 {
    ...
    function _upgradeChainFirst() internal {
        ...
        for (uint256 i = 0; i < chains.length; i++) {
            ...
            // Mirrors the tolerant batchInbox() read in OPCMUpgradeV800: the getter is removed as of
            // SystemConfig 4.0.0, so resolve it to address(0) when the selector is missing.
