...
Make sure your hardware wallet is still unlocked and run the following commands depending on which Safe you are signing on behalf of.
...
**Note:** To use Trezor instead of Ledger, add `WALLET_TYPE=trezor` to your command.
...
# With Trezor:
WALLET_TYPE=trezor just --dotenv-path $(pwd)/.env simulate council

# Without hardware wallet (development/testing):
...
# With Trezor:
WALLET_TYPE=trezor just --dotenv-path $(pwd)/.env simulate foundation
```
...
# With Trezor:
WALLET_TYPE=trezor just --dotenv-path $(pwd)/.env simulate chain-governor
```
...
# With Trezor:
WALLET_TYPE=trezor just --dotenv-path $(pwd)/.env sign council

# With keystore:
WALLET_TYPE=keystore just --dotenv-path $(pwd)/.env sign council

# With different HD path:
HD_PATH=1 just --dotenv-path $(pwd)/.env sign council
```
...
# With Trezor:
WALLET_TYPE=trezor just --dotenv-path $(pwd)/.env sign foundation
```
...
# With Trezor:
WALLET_TYPE=trezor just --dotenv-path $(pwd)/.env sign chain-governor
```
