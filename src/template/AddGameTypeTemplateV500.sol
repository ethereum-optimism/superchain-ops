...
/// @title IOPContractsManagerU18
/// @notice U18 compatible OPCM interface
interface IOPContractsManagerU18 {
    ...
    function addGameType(AddGameInput[] memory inputs) external;
}

/// @title AddGameTypeTemplateV500
/// @notice This template is used to add a game type to the DisputeGameFactory contract.
///         This template is compatible with U18+.
contract AddGameTypeTemplateV500 is OPCMTaskBase {
    ...
    function _build(address) internal override {
        ...
        for (uint256 i = 0; i < chains.length; i++) {
            ...
            // TODO: This is commented out to allow devnets to deploy a permissionless weth proxy by setting a zero weth address
        ...
        console.log("[LOG] code.size.length %d", address(OPCM).code.length);
    ...
    function _validate(VmSafe.AccountAccess[] memory, Action[] memory, address) internal view override {
        ...
        for (uint256 i = 0; i < chains.length; i++) {
            ...
            if (address(cfg[chainId].delayedWETH) == address(0)) {
                // A new weth proxy must be deployed if the weth input is zero
                assertNotEq(address(game.weth()), address(0));
            }
