...
/// Supports: op-contracts/v7.1.16
contract OPCMUpgradeV710 is OPCMTaskBase {
    ...
    IOPContractsManagerV710 public opcm;
    ...
    function _templateSetup(string memory taskConfigFilePath, address rootSafe) internal override {
        ...
        require(chains.length > 0, "OPCMUpgradeV710: no chains configured");

        // Load upgrades from TOML
        OPCMUpgrade[] memory _upgrades = abi.decode(tomlContent.parseRaw(".opcmUpgrades"), (OPCMUpgrade[]));
        require(_upgrades.length == chains.length, "OPCMUpgradeV710: opcmUpgrades length mismatch");
        for (uint256 i = 0; i < _upgrades.length; i++) {
            require(_upgrades[i].chainId != 0, "OPCMUpgradeV710: chainId cannot be zero");
            require(upgrades[_upgrades[i].chainId].chainId == 0, "OPCMUpgradeV710: duplicate chain config");
            require(Claim.unwrap(_upgrades[i].cannonPrestate) != bytes32(0), "OPCMUpgradeV710: cannonPrestate is zero");
            require(
                Claim.unwrap(_upgrades[i].cannonKonaPrestate) != bytes32(0),
                "OPCMUpgradeV710: cannonKonaPrestate is zero"
            );
        ...
        require(superchainConfig != address(0), "OPCMUpgradeV710: SuperchainConfig not found");
        require(superchainConfig.code.length > 0, "OPCMUpgradeV710: SuperchainConfig has no code");
        for (uint256 i = 0; i < chains.length; i++) {
            uint256 chainId = chains[i].chainId;
            require(upgrades[chainId].chainId != 0, "OPCMUpgradeV710: config not found for chain");
            require(
                superchainAddrRegistry.getAddress("SuperchainConfig", chainId) == superchainConfig,
                "OPCMUpgradeV710: all chains must share the same SuperchainConfig"
            );
        }

        // Register EthLockboxProxy for each chain from the superchain-registry addresses.json.
        // The V710 upgrade writes to EthLockboxProxy storage, but it is not discovered by the
        ...
        // OPCM from TOML; must be v7.1.16
        opcm = IOPContractsManagerV710(tomlContent.readAddress(".addresses.OPCM"));
        OPCM_TARGETS.push(address(opcm));
        require(opcm.version().eq("7.1.16"), "Incorrect OPCM");
    ...
    function _buildOneGameConfig(
        ...
        uint256 bond
    ) internal view returns (IOPContractsManagerV710.DisputeGameConfig memory) {
        ...
        return IOPContractsManagerV710.DisputeGameConfig({
            enabled: enabled,
    ...
    function _buildGameConfigs(uint256 chainId)
        internal
        view
        returns (IOPContractsManagerV710.DisputeGameConfig[] memory)
    {
        ...
        IOPContractsManagerV710.DisputeGameConfig[] memory cfgs = new IOPContractsManagerV710.DisputeGameConfig[](6);
        uint32[6] memory gts =
    ...
    function _buildExtraInstructions(uint256 chainId)
        internal
        view
        returns (IOPContractsManagerV710.ExtraInstruction[] memory)
    {
        IOPContractsManagerV710.ExtraInstruction[] memory extraInstructions =
            new IOPContractsManagerV710.ExtraInstruction[](2);
        extraInstructions[0] =
            IOPContractsManagerV710.ExtraInstruction({key: "PermittedProxyDeployment", data: bytes("DelayedWETH")});
        extraInstructions[1] = IOPContractsManagerV710.ExtraInstruction({
    ...
    function _build(address) internal override {
        ...
        (bool scOk,) = address(opcm).delegatecall(
            abi.encodeCall(
                IOPContractsManagerV710.upgradeSuperchain,
                (
                    IOPContractsManagerV710.SuperchainUpgradeInput({
                        superchainConfig: ISuperchainConfig(sc),
                        extraInstructions: new IOPContractsManagerV710.ExtraInstruction[](0)
                    })
                )
            )
        );
        require(scOk, "OPCMUpgradeV710: upgradeSuperchain delegatecall failed");

        for (uint256 i = 0; i < chains.length; i++) {
            uint256 chainId = chains[i].chainId;
            require(upgrades[chainId].chainId != 0, "OPCMUpgradeV710: Config not found for chain");

            IOPContractsManagerV710.UpgradeInput memory inp = IOPContractsManagerV710.UpgradeInput({
            ...
            (bool ok,) =
                address(opcm).delegatecall(abi.encodeWithSelector(IOPContractsManagerV710.upgrade.selector, inp));
            require(ok, "OPCMUpgradeV710: Delegatecall failed in _build.");
        }
    }
...
interface IOPContractsManagerV710 {
