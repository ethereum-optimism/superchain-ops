...
/// @notice OPCM SuperchainConfig V5.0.0 upgrade template.
/// @dev This template is used to upgrade the SuperchainConfig contract from v4.1.0 to V5.0.0.
/// Supports: op-contracts/v5.0.0
contract OPCMUpgradeSuperchainConfigV500 is OPCMTaskBase {
    ...
    function _build(address) internal override {
        (bool success,) = OPCM_TARGETS[0].delegatecall(
            abi.encodeCall(
                IOPContractManagerV500.upgradeSuperchainConfig, (SUPERCHAIN_CONFIG, SUPERCHAIN_CONFIG_PROXY_ADMIN)
            )
        );
        require(success, "OPCMUpgradeSuperchainConfigV500: Delegatecall failed in _build.");
    }
