...
contract OPCMUpgradeV800 is OPCMTaskBase {
    ...
    function _templateSetup(string memory taskConfigFilePath, address rootSafe) internal override {
        ...
        for (uint256 i = 0; i < chains.length; i++) {
            ...
            // SystemConfig 4.0.0 (op-contracts/v8.0.0) removes the batchInbox() getter and clears the
            // legacy slot during reinitialization, so read it with a staticcall that tolerates both
            // pre- and post-removal versions. A missing selector reverts with empty return data; any
            // other failure is unexpected and must surface loudly rather than be mistaken for the
            // post-removal case. See https://github.com/ethereum-optimism/optimism/issues/21614.
