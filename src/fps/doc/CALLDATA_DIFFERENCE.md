...
command:
```bash
forge script src/fps/example/template/GasConfigTemplate.sol --sig "run(string)" src/fps/example/task-03/taskConfig.toml src/fps/example/task-00/opmainnetConfig.toml --rpc-url mainnet -vvv
```
