...
simulate_task() {
    ...
    if [ "$is_nested" = "true" ]; then
        ...
        SIMULATE_WITHOUT_WALLET=1 just --dotenv-path "$(pwd)"/.env --justfile "$just_file" simulate "$nested_safe_name_depth_1" "$nested_safe_name_depth_2"
    else
        echo "Simulating single task: $task"
        SIMULATE_WITHOUT_WALLET=1 just --dotenv-path "$(pwd)"/.env --justfile "$just_file" simulate
    fi
