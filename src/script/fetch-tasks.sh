...
check_status() {
  ...
  # If status is not EXECUTED, CANCELLED, APPROVED, or DEVNET, add to tasks to run.
  # DEVNET marks tasks generated by `just gen-devnet-task` — they live alongside
  # production tasks under <network>/ but target a devnet, so they must not be
  # pulled into stacked simulations of unrelated work.
  if [[ "$status_line" != *"EXECUTED"* ]] && [[ "$status_line" != *"CANCELLED"* ]] && [[ "$status_line" != *"APPROVED"* ]] && [[ "$status_line" != *"DEVNET"* ]]; then
