# 056-gas-params-op-target: (Ethereum) Increase gas *target* (P0 contingency) : OP Mainnet
...
* gasLimit: 80_000_000 (unchanged; set by `055-gas-params-op-p0`)
...
after `055-gas-params-op-p0` (which raised the gas limit to 40Mgas/s while holding the
...
> [!IMPORTANT]
> This task assumes `055-gas-params-op-p0` has already executed (gasLimit 80M,
> elasticity 4). The `config.toml` reproduces that post-055 state via `SystemConfig`
> storage overrides so the simulated diff shows the only real change (elasticity 4 → 2).
> If 055 has not executed, the on-chain pre-state differs and the hashes/diff below must
> be regenerated.
...
Executed only if monitoring after 055 shows the 10Mgas/s gas target is being reached and
...
Simulation commands:
```bash
cd src/tasks/eth/056-gas-params-op-target
