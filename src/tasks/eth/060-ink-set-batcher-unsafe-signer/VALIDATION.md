...
> - Message Hash: `0xbb8b508e818b3ed670c792de34fd302775542f26fd39d2822c726825588a2e31`
> - Safe Hash:    `0x357bc05a6de22301d0371f39c8dc7da189692aba00cec5fbc1344aade89189ff`
>
> _Generated by `just simulate` with the [config.toml](./config.toml) state overrides (FoundationOperationsSafe nonce = 118; `SystemConfig.owner` → FOS). Valid only for those overrides — re-run `just simulate` and replace them if the FOS nonce has advanced or the owner override is removed (after the ownership transfer executes) before signing._
>
...
Nonce increments `118` → `119`. (Simulation labels this address `Challenger (GnosisSafe) - Chain ID: 10` — the same Safe is the OP Mainnet challenger in the registry; the contract being modified here is the Ink SystemConfig owner Safe.)
