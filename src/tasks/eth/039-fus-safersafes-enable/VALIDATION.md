...
This document provides a detailed analysis of the final calldata executed on-chain for enabling SaferSafes on the Foundation Upgrade Safe.
...
1. **Set Guard to zero** - Clears any existing guard (already zero for FuS)
...
|------|--------|-------------|
| 1 | `0x847B5c174615B1B7fDF770882256e2D3E95b9D92` | FuS: setGuard(address(0)) |
| 2 | `0x847B5c174615B1B7fDF770882256e2D3E95b9D92` | FuS: enableModule(SaferSafes) |
...
| Key | Before | After | Description |
|-----|--------|-------|-------------|
| `0x408d52a038ff1186c592b8427b27cd6d9ba34b24b514682ed0b0cd3d9dada1e3` | `0x0` | `0x278d00` | `livenessSafeConfiguration[FuS].livenessResponsePeriod` = 2592000 (30 days) |
| `0x408d52a038ff1186c592b8427b27cd6d9ba34b24b514682ed0b0cd3d9dada1e4` | `0x0` | `0x...c2819dc788505aac350142a7a707bf9d03e3bd03` | `livenessSafeConfiguration[FuS].fallbackOwner` = Security Council |

#### Slot Derivation

The SaferSafes contract stores liveness configuration in a mapping. The base slot for FuS's configuration is computed as:

```
baseSlot = keccak256(abi.encode(fusAddress, mappingSlot))
```
