...
This is task 4 of 4 in a signer rotation split into single-owner swaps (see [`055-fus-rotation-1`](../055-fus-rotation-1) for the full sequence). It executes after [`057-fos-rotation-1`](../057-fos-rotation-1).

> [!IMPORTANT]
>
> Owner `0xf1EfbdC2C0BDC4554E0f1639D7fe88cD870a4639` (added to the Foundation Operations Safe in [`057-fos-rotation-1`](../057-fos-rotation-1)) **MUST be one of the signers for this task**, alongside 4 other current owners to reach the threshold of 5. This produces a real Safe signature from the new key, operationally verifying it.
...
```bash
cd src
SIMULATE_WITHOUT_LEDGER=1 just simulate-stack eth 058-fos-rotation-2
```
...
```bash
cd src
just sign-stack eth 058-fos-rotation-2
```
