...
1. `080-migrations-sop-1-add-game-type`
2. `081-migrations-sop-1-set-respected-game-type`
3. `082-migrations-sop-1-transfer-system-config-owner` (Safe-signed by Safe A → OPE Receiving Safe / Safe B)
4. `083-migrations-sop-1-set-batcher-unsafe-signer`
5. `084-migrations-sop-1-proposer-rotation`
6. **`085-migrations-sop-1-fee-vault-update`** ← this task

## Simulation & Signing

Simulation commands:
```bash
cd src/tasks/sep/085-migrations-sop-1-fee-vault-update
