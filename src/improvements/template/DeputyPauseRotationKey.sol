...
contract DeputyPauseRotationKey is L2TaskBase {
    ...
    struct DeputyPauseModuleSetDeputyInputs {
        address new_deputy;
    ...
    function _templateSetup(string memory taskConfigFilePath) internal override {
        ...
        DeputyPauseModuleSetDeputyInputs memory inputs = abi.decode(
            vm.parseBytes(vm.parseToml(taskConfigFilePath).get(".inputs")), 
            (DeputyPauseModuleSetDeputyInputs)
        );

        // Store the inputs
        new_deputy = inputs.new_deputy;
        new_deputy_signature = inputs.new_deputy_signature;
    }
    ...
    function _build() internal override {
        ...
        assertEq(dpm.deputy(), address(0), "ERR101: DeputyPauseModule should have no deputy");
