...
// Interface for old liveness module (if needed for clearing)
...
contract TransitionToLivenessModule2 is SimpleTaskBase {
    ...
    function _setAllowedStorageAccesses() internal override {
        ...
        _allowedStorageAccesses.add(currentModule);
    ...
    function _templateSetup(string memory taskConfigFilePath, address rootSafe) internal override {
        ...
        currentModule = vm.parseTomlAddress(file, ".currentModule");
    ...
    function _build(address rootSafe) internal override {
        ...
        ModuleManager(rootSafe).disableModule(previousModule, currentModule);

        // Step 2: Deactivate the LivenessGuard by setting guard to address(0)
        GuardManager(rootSafe).setGuard(address(0));

        // Step 3: Enable the new LivenessModule2
        ModuleManager(rootSafe).enableModule(newModule);

        // Step 4: Configure the new LivenessModule2
        ...
        override
    {
        ...
        _validateGuardDisabled(rootSafe);
    ...
    function _validateStorageWrites(VmSafe.AccountAccess[] memory accountAccesses, address rootSafe) internal view {
        bytes32 currentModuleSlot = keccak256(abi.encode(currentModule, MODULE_MAPPING_STORAGE_OFFSET));
        bytes32 newModuleSlot = keccak256(abi.encode(newModule, MODULE_MAPPING_STORAGE_OFFSET));
        ...
        for (uint256 i = 0; i < uniqueWrites.length; i++) {
            ...
            if (uniqueWrites[i] == currentModule) {
    ...
    function _validateSafeStorageWrites(
        ...
        for (uint256 i = 0; i < accountWrites.length; i++) {
            ...
            // Validate guard disabled
            if (storageAccess.slot == GUARD_STORAGE_SLOT) {
                assertEq(address(uint160(uint256(storageAccess.newValue))), address(0), "guard not disabled");
                guardDisableFound = true;
            }
        }
        ...
        assertTrue(guardDisableFound, "Guard disable write not found");
    }
