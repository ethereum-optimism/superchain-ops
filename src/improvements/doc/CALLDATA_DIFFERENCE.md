...
command:
```bash
forge script test/task/mock/example/template/GasConfigTemplate.sol --sig "runSimulate(string)" test/task/mock/example/task-03/mainnetConfig.toml --rpc-url mainnet -vvv
```
