...
contract StackedSimulator is Script {
    ...
    function simulateStack(string memory network, string memory task, address childSafeDepth2, address childSafeDepth1)
        public
    {
        require(childSafeDepth1 != address(0) || childSafeDepth2 != address(0), "StackedSimulator: At least one child safe must be provided.");
        address[] memory childSafes;
        if (childSafeDepth2 != address(0)) {
            require(childSafeDepth1 != address(0), "StackedSimulator: childSafeDepth1 must be provided when childSafeDepth2 is specified.");
